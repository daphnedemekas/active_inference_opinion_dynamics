...
def update_posterior_states_factorized(obs, A_reduced, informative_dims, n_states, prior=None, **inference_params):
    """
    Update marginal posteriors over hidden states using variational inference
        Can optionally set message passing algorithm used for inference.
        @ NOTE: New version introduced on 18.05.2021 as part of the active inference opinion dynamics package, 
        accelerated version that takes advantage of the sparsity of the factor graph that links
        particular hidden state factors to eachother.
    Parameters
    ----------
    - 'obs' [numpy 1D array, array of arrays (with 1D numpy array entries), int or tuple]:
        The observation (generated by the environment). If single modality, this can be a 1D array
    ...
    - `n_states` [list :: int]:
        This is a list of dimensionalities of the hidden state factor marginals.
    - 'prior' [numpy 1D array, array of arrays (with 1D numpy array entries), Categorical, or None]:
        Prior beliefs about hidden states, to be integrated with the marginal likelihood to obtain
         a posterior distribution.
        If None, prior is set to be equal to a flat categorical distribution (at the level of
    ...
    # safe convert to numpy
    A_reduced= utils.to_numpy(A_reduced)

    # collect model dimensions
    if utils.is_arr_of_arr(A_reduced):
        n_modalities = len(A_reduced)
        n_observations = [a_m.shape[0] for a_m in A_reduced]
    else:
        n_modalities = 1
        n_observations = [A_reduced.shape[0]]

    obs = utils.process_observation(obs, n_modalities, n_observations)
    if prior is not None:
        prior = utils.process_prior(prior)

    qs = fpi.run_fpi_factorized(obs, A_reduced, informative_dims, n_states, prior = prior, **inference_params)
