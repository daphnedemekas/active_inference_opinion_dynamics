...
def run_sweep(param_combos):
    ...
    for p_idx, param_config in enumerate(param_combos):
        ...
        if not os.path.isdir('Analysis/results/ecb_connectivity_results/' + str(p_idx)  +"/"):
            os.mkdir('Analysis/results/ecb_connectivity_results/' + str(p_idx)+"/")
        ...
        for trial_i in range(n_trials):
            ...
            np.savez('Analysis/results/ecb_connectivity_results/' + str(p_idx) + "/" + str(trial_i) , trial_results)
...
if __name__ == '__main__':
    ...
    connectedness_values = np.linspace(0.05,0.8,15)
