...
class GenerativeModel(object):
    ...
    def __init__(
        ...
        self.num_topics = num_neighbours
        ...
        #The idea mapping now has to no longer map tweet -- > belief but from tweet about agent --> belief that that is the imposter 
        
        
        
        self.num_obs = [self.num_topics] + (self.num_neighbours) * [self.num_topics+1] + [self.num_neighbours]                               # list that contains the dimensionalities of each sensory   

        self.num_modalities = len(self.num_obs) # total number of observation modalities
        self.num_states = (1+ self.num_neighbours) * [self.idea_levels] + [self.num_topics] + [self.num_neighbours]
        ...
        num_controls[self.h_control_idx] = self.num_topics #i can tweet that its any one of my neighbours
    ...
    def generate_likelihood(self):
        ...
        for o_idx, o_dim in enumerate(self.num_obs):
            ...
            if o_idx == self.focal_h_idx: #this happens for o_idx == 0 -- we are in the 0th sensory modality (my beliefs about H) 
                #if i know i am the imposter than this totally changes my generative model 

                h_obs = self.num_topics #i observe my own tweets
                ...
                for item in itertools.product(*[list(range(d)) for d in fill_dimensions]):
                    slice_ = list(item)
                    A_indices = insert_multiple(slice_, fill_indices, [slice(0,h_obs), slice(0,self.num_topics)]) #here we insert the correct values for the fill indices for this slice                    
