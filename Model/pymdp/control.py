...
def update_posterior_policies2(
    ...
    """
    n_policies = len(agent.genmodel.policies)
    ...
    for idx, policy in enumerate(agent.genmodel.policies):
        ...
        if use_utility:
            neg_efe[idx] += calc_expected_utility(qo_pi, agent.genmodel.C)

        if use_states_info_gain:
            neg_efe[idx] += calc_states_info_gain(agent, qs_pi)

    q_pi = softmax(gamma*neg_efe + agent.genmodel.E)
...
def calc_states_info_gain(agent, qs_pi):
    """
