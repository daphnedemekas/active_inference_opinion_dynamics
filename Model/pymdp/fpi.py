...
import numpy as np
from .maths import spm_dot, get_joint_likelihood, softmax, calc_free_energy



def run_fpi2(agent, obs, prior=None):
    """
    Update marginal posterior beliefs about hidden states using variational fixed point iteration (FPI).
   
    Parameters
    ----------
    - 'agent' the agent which holds all needed parameters
    ...
    num_iter, dF, dF_tol = agent.inference_params["num_iter"], agent.inference_params["dF"], agent.inference_params["dF_tol"]
    n_factors, n_modalities, n_states = agent.genmodel.num_factors, agent.genmodel.num_modalities, agent.genmodel.num_states
    ...
    likelihood = get_joint_likelihood(agent.genmodel.A, obs, n_states)
